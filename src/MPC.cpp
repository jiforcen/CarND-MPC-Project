...
class FG_eval {
 public:
  ...
  void operator()(ADvector& fg, const ADvector& vars) {
    ...
    const int delta_seq_weight = 150;
    const int a_seq_weight = 10;
    ...
    for (int t = 0; t < N - 2; t++) {
      fg[0] += delta_seq_weight * CppAD::pow(vars[delta_start + t + 1] - vars[delta_start + t], 2);
      fg[0] += a_seq_weight * CppAD::pow(vars[a_start + t + 1] - vars[a_start + t], 2);
    }
